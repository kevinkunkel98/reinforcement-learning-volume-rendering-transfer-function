"""SAC training CLI for `VisibilityTFEnv`, across the TotalSegmentator
training volumes, with periodic evaluation on the validation volumes.
...
import datasets
import goals
from rl.vis_env import VisibilityTFEnv
...
def build_env(volume_ids=None, model_for_volume=None) -> VisibilityTFEnv:
    """The training environment: `VisibilityTFEnv` over the training split,
    or `volume_ids` when given (tests inject a stub volume list)."""
    ...
    return VisibilityTFEnv(ids, **kwargs)


def build_eval_env(volume_ids=None, model_for_volume=None) -> VisibilityTFEnv:
    """The validation environment: `VisibilityTFEnv` over the validation
    split, or `volume_ids` when given."""
...
def evaluate_policy_on(episodes, model) -> list:
    """Run each `(env, seed)` episode to completion with the policy's
    deterministic action; return one `{"attainment", "kind"}` dict per
    episode, from the final step's info."""
    results = []
...
def run_training(out: str, timesteps: int, seed: int, eval_interval: int,
    ...
    model = SAC("MlpPolicy", train_env, seed=seed, verbose=1)
...
def main(argv=None):
    ...
    print(f"\n[vis_train] wrote {result['eval_progress_path']} and {result['best_path']}")
